...
class InclusionExclusionSignature(dspy.Signature):
    """Signature for inclusion and exclusion questions."""
    inclusion_questions: dict = {
        'Amnesia case report? (Y/N)': 'case_report',
        'Published in English? (Y/N)': 'is_english'
    }
    exclusion_questions: dict = {
...
class DataExtractionSignature(dspy.Signature):
    """Signature for data extraction questions."""
    strict_extraction_questions: dict = {
    ...
    lenient_extraction_questions: dict = {
...
class QuestionTemplateModule(dspy.Module):
    """DSPy module for managing question templates."""

    def __init__(self):
        super().__init__()
        self.inclusion_exclusion = InclusionExclusionSignature()
        self.data_extraction = DataExtractionSignature()

    def print_inclusion_exclusion(self):
        print("Here are example inclusion questions:")
        print(self.inclusion_exclusion.inclusion_questions)
        print("Here are example exclusion questions:")
        print(self.inclusion_exclusion.exclusion_questions)

    def print_data_extraction(self):
        print("Here are example data extraction questions:")
        print(self.data_extraction.strict_extraction_questions)
        print("Here are some more generalizable data extraction questions:")
        print(self.data_extraction.lenient_extraction_questions)
        print("Here is a question template")
        print(self.data_extraction.template_questions)

# Example usage:
# qt = QuestionTemplateModule()
# qt.print_inclusion_exclusion()
# qt.print_data_extraction()