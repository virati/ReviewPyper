...
class OpenAIJsonEvaluator(OpenAIChatBase):
    def __init__(
        ...
        """
        super().__init__(
            api_key_path, question_type=question_type, model_choice=model_choice
        )
    ...
    def save_to_json(self, output_dict):
        ...
        while os.path.exists(save_file):
            save_file = os.path.join(
                out_dir, f"{self.question_type}_evaluations_{count}.json"
            )
            count += 1
            ...
            for file_name, file_text in tqdm(self.relevant_text_by_file.items()):
                ...
                for chunk_index, chunk in enumerate(
                    ...
                    for q_index, q in enumerate(
                        ...
                        conversation = self.generate_submission(
                            chunk, q
                        )  # Generate the conversation to submit
                        answer, tokens_used = self.evaluate_with_openai(
                            conversation
                        )  # Evaluate the chunk with OpenAI
                        self.all_answers[file_name][q][f"chunk_{chunk_index + 1}"] = (
                            answer  # Store the answer for this question and this chunk
                        )
