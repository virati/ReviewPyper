...
from revpyper.utils.txt import TextChunker
from revpyper.chatgpt.gpt_utils.openai_labeller import CaseReportLabeler
from revpyper.chatgpt.gpt_utils.openai_summarizer import OpenAISummarizer
...
class SectionLabeler:
    ...
    def select_labels(self):
        ...
        if self.article_type == "research":
            ...
            self.section_headers = {"Case_Report": ["yes", "y", "positive", "correct"]}
        elif self.article_type == "other":
            self.section_headers = {"Positive": ["yes", "y", "positive", "correct"]}
        else:
            raise ValueError(
                f"Unknown article type {self.article_type}, choose case or research."
            )
    ...
    def label_with_loose_matching(self, text, labeled_sections):
        ...
        for line in text.split("\n"):
            for section, labels in self.section_headers.items():
                if any(
                    re.search(f"\\b{label}\\b", line, re.IGNORECASE) for label in labels
                ):
                    if current_section:
                        if current_section not in labeled_sections:
                            labeled_sections[current_section] = current_text.strip()
    ...
    def label_text(self, text, show_residuals=True):
        ...
        if len(residual_text) > 100:
            print(
                f"Warning: High number of characters missed ({len(residual_text)}), please investigate manually."
            )
            if show_residuals:
                print(residual_text)
    ...
    def save_to_json(self, output_dict, filename=None):
        ...
        if filename is not None and not os.path.exists(filename):
            ...
            save_file_path = os.path.join(
                out_dir, f"_{self.article_type}_labeled_sections.json"
            )
            with open(save_file_path, "w") as f:
                json.dump(output_dict, f, indent=0)
                print(f"Saved to: \n {save_file_path}")
    ...
    def _label_sections(self, text, question=None):
        ...
        if self.article_type == "research":
            ...
            raise ValueError(
                f"Unknown article type {self.article_type}, choose 'case', 'research', or 'other'"
            )
    ...
    def _json_file_exists(self, filename):
        ...
        json_filename = os.path.join(
            out_dir, f"{os.path.splitext(filename)[0]}_labeled_sections.json"
        )
...
class FilterPapers:
    ...
    def filter_json(self):
        ...
        """
        filtered_data = {
            key: value for key, value in self.data.items() if key in self.df.index
        }
    ...
    def save_to_json(self):
        ...
        out_dir = os.path.join(
            os.path.dirname(self.csv_path), "inclusion_exclusion_json"
        )
        os.makedirs(out_dir, exist_ok=True)
...
class InclusionExclusionSummarizer:
    ...
    def summarize_results(self):
        ...
        for article, questions in self.data.items():
            ...
            for question, chunks in questions.items():
                ...
                if polarity is None:
                    raise ValueError(
                        f"The question from the JSON: \n\n'{question}' \n\n was not found in the questions dictionary."
                    )

                # Convert all chunk answers to lowercase and check for "yes" keywords
                binary_answers = [
                    polarity
                    if any(s in answer.lower() for s in self.acceptable_strings)
                    else abs(1 - polarity)
                    for answer in chunks.values()
                ]
    ...
    def run(self):
        ...
        print(
            f"Your CSV files of filtered manuscripts have been saved to this directory: \n {os.path.dirname(raw_path)}"
        )
...
class CustomSummarizer(InclusionExclusionSummarizer):
    ...
    def __init__(
        self, json_path, answers_binary=False, api_key_path=None, summary_type="llm"
    ):
        """
        ...
        for article, questions in self.data.items():
            ...
            for question, chunks in questions.items():
                ...
                if self.keyword_mapping:
                    mapped_answers = [
                        self.keyword_or_fuzzy_match(answer)
                        for answer in chunks.values()
                    ]
                    if all(x is np.nan for x in mapped_answers) or all(
                        x is None for x in mapped_answers
                    ):
                        summary_dict[article][question] = np.nan
                    else:
                        valid_answers = [
                            x
                            for x in mapped_answers
                            if x is not np.nan and x is not None
                        ]
                        summary_dict[article][question] = (
                            np.sum(valid_answers) if valid_answers else "Unidentified"
                        )
    ...
    def summarize_with_llm(self):
        summary_dict = {}
        for article, questions in tqdm(
            self.data.items(), desc="Summarizing final responses with LLM"
        ):
            summary_dict[article] = {}
            for question, chunks in questions.items():
    ...
    def run_custom(self):
        ...
        automated_path = self.save_to_csv(
            filename=f"responses_claned_with_{self.summary_type}"
        )
        print(
            f"Your CSV files of filtered manuscripts have been saved to this directory: \n {os.path.dirname(raw_path)}"
        )
